...
public class BoardController {
	...
	public String list(Model model, String page) {
		...
		int end = nowPage + Common.Board.BLOCKLIST - 1;
	...
	public String reply(int idx, String name, String subject, String content, String pwd, String ip, int page) {
	 ...
	 * //게시물 상세 조회하기
	 * 
	 * @RequestMapping("/view.do") public String view(Model model, int idx, int
	 * page) {
	 * 
	 * //idx에 해당하는 게시글 한 건 얻어오기 BoardVO vo = board_dao.selectOne(idx);
	 * 
	 * //조회수 증가 (F5를 연타했을 때 조회수가 급상승 하는걸 방지) HttpSession session =
	 * request.getSession(); String show = (String)session.getAttribute("show");
	 * 
	 * if(show == null) { int res = board_dao.update_readhit(idx); //세션에 "show"라는
	 * 이름으로 저장 session.setAttribute("show", "0"); }
	 * 
	 * //바인딩 model.addAttribute("vo",vo);
	 * 
	 * //포워딩 return Common.VIEW_PATH+"board_view.jsp";
	 * 
	 * 
	 * @RequestMapping("/reply_form.do") public String reply_form(int idx, int page)
	 * { return Common.VIEW_PATH + "reply_form.jsp?idx="+idx+"&page="+page; }
	 * 
	 * }
	 * 
	 */
	  ...
	  public String modify(Model model, BoardVO vo,int idx) {
		  BoardVO baseVo = board_dao.selectOne(idx);
	  ...
	  int res = board_dao.board_update(baseVo);
		...
		// 포워딩
		return Common.VIEW_PATH + "board_list.jsp?idx="+idx+"&page="+page;
